...
These corrections are also eligible for Git notes. Notes are local unless
`refs/notes/commits` is explicitly published; this checkpoint does not push
either the branch or notes.
...
| `src/core/sparsity.ts:51` | “SHA-256 style 256-bit hash” | Eight bespoke 32-bit accumulators are not SHA-256 or a reviewed cryptographic hash. | known-open | QUARANTINED |
